...
import {
  ...
  BookOpen, DollarSign, RefreshCw, Edit2, X, Loader2,
  Upload, CheckCircle2, Trash2, Code2,
} from "lucide-react";
...
function UpdateProfileModal({ profile, onClose, onSuccess }) {
  ...
  const addLoc = () => {
    ...
    if (l && !preferredLocation.includes(l)) setValue("preferredLocation", [...preferredLocation, l]);
    setLocInput("");
  };
  const removeLoc = (l) => setValue("preferredLocation", preferredLocation.filter((x) => x !== l));
  ...
  const handleImgUpload = () =>
    uploadFile(imgFileRef, {
      label: "Photo",
      onUrl: (url) => { setValue("profileImage", url); setImgUrl(url); setImgPreview(url); },
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            <div className="grid sm:grid-cols-3 gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <label className="flex-1 cursor-pointer">
                      <div className={`h-9 px-2.5 rounded-lg bg-card border text-xs flex items-center gap-1.5 overflow-hidden ${
                        imgUploaded ? "border-green-500/60" : "border-border"
                      }`}>
                        {imgUploaded ? <CheckCircle2 className="h-3.5 w-3.5 text-green-500 shrink-0" />
                          : <Upload className="h-3.5 w-3.5 text-muted-foreground shrink-0" />}
                        <span className="truncate text-muted-foreground">{imgFileName || "Choose image…"}</span>
                      </div>
                      <input type="file" accept="image/*" className="hidden" onChange={handleImgChange} />
                    </label>
                    <button type="button" disabled={!imgFileRef || imgUploading || imgUploaded} onClick={handleImgUpload}
                      className="btn-primary text-xs px-3 h-9 shrink-0 disabled:opacity-40 flex items-center gap-1">
                      {imgUploading ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Uploading…</>
                        : imgUploaded ? <><CheckCircle2 className="h-3.5 w-3.5" /> Uploaded</>
                ...
                {imgUrl && (
                  <div className="mt-1 flex items-center gap-1 text-[10px] text-green-600">
                    ...
                    <a href={imgUrl} target="_blank" rel="noopener noreferrer"
                      className="truncate hover:underline">{imgUrl}</a>
                  </div>
                )}
              ...
              <div className="sm:col-span-3">
                <FL label="Headline">
                  <input {...register("headline")} placeholder="Full Stack Developer | MERN"
                    className={inputCls()} />
                </FL>
              </div>
              ...
              <div className="sm:col-span-3">
                <FL label="Bio">
                  <textarea {...register("bio")} rows={2} placeholder="A short bio…"
              ...
              <div className="sm:col-span-2">
                <FL label="Resume URL">
                  <input {...register("resumeUrl")} placeholder="https://…/resume.pdf" className={inputCls()} />
                </FL>
              </div>
              <FL label="Expected Salary (₹)">
                <input type="number" {...register("expectedSalary")} placeholder="1200000" className={inputCls()} />
              </FL>
                ...
                <FL label="Preferred Locations">
                  <div className="flex gap-2">
                    <input value={locInput} onChange={(e) => setLocInput(e.target.value)}
                      ...
                      placeholder="e.g. Bengaluru" className="flex-1 h-9 px-2.5 rounded-lg bg-card border border-border text-xs focus:outline-none focus:ring-2 focus:ring-primary/40" />
                    <button type="button" onClick={addLoc} className="btn-primary text-xs px-3 h-9">Add</button>
                  </div>
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {preferredLocation.map((l) => (
                      <span key={l} className="chip text-xs inline-flex items-center gap-1">
                        {l}
                        <button type="button" onClick={() => removeLoc(l)} className="hover:text-destructive">
              ...
              <div className="sm:col-span-3">
                <FL label="Skills">
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={addSkill} className="btn-primary text-xs px-3 h-9">Add</button>
                  </div>
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {skills.map((s) => (
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              <button type="button" onClick={onClose} className="btn-outline text-xs h-8 px-4">Cancel</button>
  ...
  if (profileError && !profile) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center px-4">
        ...
        <button onClick={() => fetchProfile().catch(() => {})}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card-soft p-6 sm:p-8">
            <div className="flex items-start gap-5 flex-wrap">
              {p.profileImage
                ? <img src={p.profileImage} alt="Avatar"
